...
class CircularNotchedRectangle extends NotchedShape {
  ...
  /// the guest circle.
  ///
  /// Parameter [isTop] allows for the inversion of the circular notch
  /// so that it may be applied to the bottom of a path when set to false
  // TODO(amirh): add an example diagram here.
  @override
  Path getOuterPath(Rect host, Rect? guest, {bool isTop = true}) {
    ...
    // multiplier to use for inverting realvent variables if notch
    // is to be applied to the bottom of a path
    final double invertMultiplier = isTop ? 1.0 : -1.0;
    ...
    final double b = (isTop ? host.top : host.bottom) - guest.center.dy;
    ...
    // use points to draw out a path object
    var path = Path();
